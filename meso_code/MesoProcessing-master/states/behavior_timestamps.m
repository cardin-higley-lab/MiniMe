function states = behavior_timestamps(spike2_data, pc, hands, state_duration, varargin)
...
if temp < 9
    ...
    elseif std(hands) > 850                     % works better for VERY aggressive groomers (ro073-105_04142022...std(pc) was >1000)
        ...
        groom_thresh = 2;
    elseif std(hands) > 600                     % works better for aggressive groomers or high saturation vids 
        hands = filtfilt(b,a,double(hands));
        hands = zscore(hands);
    ...
    groomHighOn_int=squeeze(groomHighOn(groomHighOff-groomHighOn>state_duration));
    groomHighOff_int=squeeze(groomHighOff(groomHighOff-groomHighOn>state_duration));
...
elseif  max(spike2_data.wheelSpeed) < 1
    ...
    locoBoutTime = wheelOff_final - wheelOn_final > state_duration; 
...
elseif max(spike2_data.wheelSpeed) < 1
    ...
    sitBoutTime = sitOff_final - sitOn_final > state_duration; 
...
faceHighOn_inter=squeeze(faceHighSitOn(faceHighSitOff-faceHighSitOn>state_duration));
faceHighOff_inter=squeeze(faceHighSitOff(faceHighSitOff-faceHighSitOn>state_duration));
...
faceLowSitOn_inter = squeeze(faceLowSitOn(faceLowSitOff-faceLowSitOn>state_duration));
faceLowSitOff_inter = squeeze(faceLowSitOff(faceLowSitOff-faceLowSitOn>state_duration));
...
% add to states data
states.locoOn = wheelOn_final';
states.locoOff = wheelOff_final';
