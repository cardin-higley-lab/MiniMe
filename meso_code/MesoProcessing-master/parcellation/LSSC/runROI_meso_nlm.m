...
A_full=zeros(NROWS*NCOLS,size(PixxTime_dff,2), "single");
A_full(allregionspix,:)=PixxTime_dff; clear PixxTime_dff
...
Lambda = zeros(cfg.N_TRIALS,cfg.N_EIG-1); %10x50 matrix 
...
%%
tic;
for trial_ind = 1:cfg.N_TRIALS %trials = segments of the session 
    % more internal params 
    configParams = [];
    ...
    % gets time (seconds) indices for segment working on ; 10 frames/second
...
% %%
max_Psi = max(embedding_norm_part,[],3); clear embedding_norm_part
