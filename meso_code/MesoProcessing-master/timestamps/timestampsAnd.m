function [timestampsAndOn,timestampsAndOff] = timestampsAnd(timestamps1On ,timestamp1Off,timestamps2On,timestamps2Off)
...
for i = 1:length(timestamp1Off)
    for ii = 1:length(timestamps2Off)
        [andOn,andOff] = timestampAnd(timestamps1On (i),timestamp1Off(i),timestamps2On(ii),timestamps2Off(ii));
        if ~isempty(andOn)
