...
piezo = data(:, params.channels.WHEEL);
figure(); plot(piezo)
...
for ni = 1:length(names)
    switch names{ni}
        ...
        case 'BLUE'
            ...
            channel_data.blueOnTimestamps = protoBlueOnTimestamps; 
            channel_data.blueOffTimestamps = protoBlueOffTimestamps;
        case 'UV'
            [protoUvOnTimestamps,protoUvOffTimestamps]=analog_sig_to_timestamps(data(:,params.channels.UV),channel_data.analog_signal_time_vect);
             channel_data.uvOnTimestamps = protoUvOnTimestamps;
             channel_data.uvOffTimestamps = protoUvOffTimestamps;
        case {'FRAMETICKS' 'RED_MESO'}
            ...
            channel_data.mesoFrameOnTimestamps = protoMesoFrameOnTimestamps;
            channel_data.mesoFrameOffTimestamps = protoMesoFrameOffTimestamps;
        case 'PHOTO_DIODE'
%             %timestamps = cumsum(ones(size(currDataBundle.data))/currDataBundle.samplingRate)+double(currDataBundle.acquisitionOffset);
%             timestamps = cumsum(ones(size(channel_data.analog_signal_time_vect))/currDataBundle.samplingRate)+double(currDataBundle.acquisitionOffset);
...
%             [processedSpike2Data.diodeOnTimestamps,processedSpike2Data.diodeOffTimestamps]=analogSigToTimestamps(spike2Data.(spike2Fields{i}).data,timestamps);
            diode_data = data(:,params.channels.PHOTO_DIODE);
            diode_data(1:130*5000) = nan;
            [channel_data.diodeOnTimestamps,channel_data.diodeOffTimestamps]=analogSigToTimestamps(diode_data,channel_data.analog_signal_time_vect);
            %[channel_data.diodeOnTimestamps,channel_data.diodeOffTimestamps]=analogSigToTimestamps(data(:,params.channels.PHOTO_DIODE),channel_data.analog_signal_time_vect);
        case 'WHEEL'
