function [tform,R,C] = controlPointAlignmentTransform_rlo(slice)
    ...
    shaped = reshape(slice,R,C); % if data is not downsampled, this might throw error (i.e. if not 256 by 256)
    
    maxFrame = (shaped-min(shaped,[],'all'));
