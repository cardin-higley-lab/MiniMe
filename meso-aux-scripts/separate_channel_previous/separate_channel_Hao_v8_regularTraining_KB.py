...
mouse_input = mouse_input.capitalize()
mouse_input = mouse_input[:-1] + mouse_input[-1].upper()
...
mouse_select = [folder for folder in mouse_folder if (mouse_input in folder and date_input in folder)]
...
def is_bright_enough(frame, x1, x2, y1, y2, brightness_threshold=230):
    ...
    # Return True if the average brightness of the ROI is larger than the threshold
...
def seperateGR(data_files, data_dir, seperateIndB, seperateIndG, frame_count, globalB, globalG, global_count):
    ...
    for i, filename in enumerate(data_files):
        ...
        while True:
            ...
            if is_bright_enough(frame, x1, x2, y1, y2, uppbdy):
            ...
            elif np.mean(frame[y1:y2, x1:x2])>lowbdy:
                cropped_frame = frame[y_start:y_end, x_start:x_end]
...
def sepGBChnl(full_frames, meanvalue, uppbdy, lowbdy):
    G_frames = []
    B_frames = []
...
[full_frames, frame_count, global_count] = insertEmpty(data_files[startIndex:len(data_files)], data_dir, frame_count, global_count)
...
# calculate the indicator and meanvalue of the ROI
...
[G_frames, B_frames] = sepGBChnl(full_frames, meanvalue, uppbdy, lowbdy)
