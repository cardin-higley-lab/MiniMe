...
def seperateGR(data_files, data_dir, seperateIndB, seperateIndG, frame_count):
    ...
    for i, filename in enumerate(data_files):
        ...
        while True:
            ...
            if is_bright_enough(frame, x1, x2, y1, y2):
                ...
                if frame_count>1:
                    ...
                    if frame_count-1 in seperateIndG:
                        B_frames.append(B_frames[-1])
                        seperateIndB.append(frame_count)
                    elif frame_count-1 in seperateIndB:
        ...
        cap.release()
    return np.array(B_frames), seperateIndB, np.array(G_frames), seperateIndG, frame_count
...
def process_files(files_chunk, data_dir, output_file, B_frames_full, G_frames_full, seperateIndB, seperateIndG, frame_count):
    # Your processing code here (e.g., reading, analyzing files, etc.)
    [B_frames, seperateIndB, G_frames, seperateIndG, frame_count] = seperateGR(files_chunk, data_dir, seperateIndB, seperateIndG, frame_count)
    # After seperation, save the frames into the full data
    if len(B_frames_full) == 0 or len(G_frames_full) == 0:
        B_frames_full = B_frames
        G_frames_full = G_frames
    else:
        B_frames_full = np.concatenate((B_frames_full, B_frames), axis=0)
        G_frames_full = np.concatenate((G_frames_full, G_frames), axis=0)
    ...
    return frame_count, seperateIndB, seperateIndG
...
G_frames_full = []
frame_count = 0
# Set the batch size (50 files per batch)
batch_size = 40

# Split data_files into chunks of 50 files
...
for start_idx in tqdm(range(0, num_files, batch_size)):
    end_idx = min(start_idx + batch_size, num_files)
    files_chunk = data_files[start_idx:end_idx]
    
    # Process the chunk of files and save the result
    [frame_count, seperateIndB, seperateIndG] = process_files(files_chunk, data_dir, output_file, B_frames_full, G_frames_full, seperateIndB, seperateIndG, frame_count)
