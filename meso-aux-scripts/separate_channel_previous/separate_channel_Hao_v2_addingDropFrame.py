...
print(meso_folder)
'''
fullTimeStampsFolder = os.path.join(os.path.dirname(meso_folder[0]), 'fullTimeStamps.mat')

fullTimeStamps = loadmat(fullTimeStampsFolder)
print(fullTimeStamps)

frame_check = fullTimeStamps['frameCheck']
'''
fDropFolder = os.path.join(os.path.dirname(meso_folder[0]), 'frameDrop.mat')
fDrop = loadmat(fDropFolder)
fDropData = fDrop['frameDrop'][0]
print(fDropData.shape[0])
fDropList = []
fList = []
for item in fDropData:
    frame_locs = item[0][0][0] # e.g., '6.avi'# e.g., 538
    count = item[1][0][0]       # e.g., 1
...
def seperateGR(data_files, data_dir, seperateIndB, seperateIndG, frame_count, globalB, globalG, global_count):
    ...
    for i, filename in enumerate(data_files):
            ...
            if frame_count in fList:
                for row in fDropList:
                    if row[0] == frame_count:
                        ...
                        if frame_count in seperateIndG:
                        ...
                        elif frame_count in seperateIndB:
                            for m in range(num_drop):
                                global_count += 1
                                if m%2 == 0:                                        
                                    G_frames.append(G_frames[-1])
        ...
        cap.release()
    return np.array(B_frames), seperateIndB, np.array(G_frames), seperateIndG, frame_count, globalB, globalG, global_count
